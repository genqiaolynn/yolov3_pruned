...
from prune.prune_tools import parse_moudle_defs, gather_bn_weights, obtain_bn_mask, write_cfg, save_darknet_weights
...
def prune_demo():
    ...
    CBL_idx, Conv_idx, prune_idx = parse_moudle_defs(model.module_defs)
    ...
    threshold = prune_and_eval(model, sorted_bn, hyp0['percent'], bn_weights, prune_idx)
    ...
    # print(f'保持好剪枝后的weight文件，地址: {weight_path}')

    # TODO 新版本的保存cfg和weight
    # 生成剪枝后的cfg文件并保存模型
    cfg_path = f'config/channel_{hyp0["percent"]}_{AP3.mean():.4f}.cfg'
    write_cfg(cfg_path, [model.hyperparams.copy()] + compact_model_defs)
    print(f'保持好剪枝后的cfg文件，地址: {cfg_path}')
    # weight_path = f'checkpoints/channel_strategy3_{hyp0["global_percent"]}_mAP_{AP2.mean():.4f}.pth'
    weight_path = f'checkpoints/channel_{hyp0["percent"]}_{AP3.mean():.4f}.pth'
    # torch.save(compact_model.state_dict(), weight_path)
