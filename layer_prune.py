...
from prune.prune_tools import parse_moudle_defs_layers, save_weights, write_cfg, save_darknet_weights
...
hyp0 = {
    ...
    'prune_num_layer': 8,  # 剪掉16块的意思,也就是剪掉了48层
}
# print(hyp0)
...
def get_filters_mask_layer(model, CBL_idx, prune_shortcuts):
    model_copy = deepcopy(model)
    filters_mask = []
    for idx in CBL_idx:
...
def prune_model_keep_size2(model, prune_idx, CBL_idx, CBLidx2mask):
    ...
    for i, model_def in enumerate(model.module_defs):
        ...
        if model_def['type'] == 'convolutional':
            activation = torch.zeros(int(model_def['filters']))
            if i in prune_idx:
...
def layer_prune_demo():
    ...
    CBL_idx, Conv_idx, shortcut_idx = parse_moudle_defs_layers(model.module_defs)   # 对所有的shortcut层进行剪枝
    ...
    for index, idx in enumerate(shortcut_idx):
        bn_mean[index] = model.module_list[idx][1].weight.data.abs().mean().clone()
    bn_mean_index = torch.sort(bn_mean)[1]

    prune_shortcuts = torch.tensor(shortcut_idx)[[bn_mean_index[: hyp0['prune_num_layer']]]]  # 这些都是要删掉的层，均值排序，大的gamma系数对应的保留
    ...
    index_all = [i for i in range(len(model.module_defs))]
    prune_index = []
    for idx in prune_shortcuts:
    ...
    # CBL2mask = get_filters_mask1(model, CBL_idx, prune_shortcuts)
    CBL2mask = get_filters_mask_layer(model, CBL_idx, prune_shortcuts)

    # 处理bn层的β
    # 将稀疏化训练后的模型中待剪枝层的bn中的β参数移植到下后面的层,并返回移植后的模型
    pruned_model = beta2next_(model, CBL_idx, CBL_idx, CBL2mask)
    ...
    print(f'testing the mAP of final pruned model: {AP3.mean():.4f}')
    # 比较剪枝前后参数数量的变化，指标的变化
    metric_table = [
        ["Metric", "Before", "After"],
    ...
    # 生成剪枝后的cfg文件并保存模型
    cfg_path = f'config/layer_{hyp0["prune_num_layer"]}_shortcut_{AP3.mean():.4f}.cfg'
    write_cfg(cfg_path, [model.hyperparams.copy()] + compact_module_defs)
    print(f'保持好剪枝后的cfg文件，地址: {cfg_path}')
    # weight_path = f'checkpoints/channel_strategy3_{hyp0["global_percent"]}_mAP_{AP2.mean():.4f}.pth'
    weight_path = f'checkpoints/layer_{hyp0["prune_num_layer"]}_shortcut_{AP3.mean():.4f}.pth'
    # torch.save(compact_model.state_dict(), weight_path)
    # save_weights(compact_model, path=weight_path)
